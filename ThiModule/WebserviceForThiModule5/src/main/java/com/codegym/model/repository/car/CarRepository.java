...
public interface CarRepository extends JpaRepository<Car, Integer> {
    ...
    @Query("select c from Car c where c.name like CONCAT('%',:content,'%') and c.isDeleted = false")
    List<Car> searchByName(@Param("content") String content);

    @Query("select c from Car c where c.id = :content and c.isDeleted = false")
    List<Car> searchByID(@Param("content") int content);
}